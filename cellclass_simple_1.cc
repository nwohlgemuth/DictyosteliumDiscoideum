...
void Cell::calculate_cadherin_force(int cadherin_index) { // WHAT IS THE POINT OF THIS FUNCTION???
	return;
}
...
void Cell::calculate_cadherin_front_indicator(int cadherin_index) {
	double tmp = cadherin[cadherin_index].vector[0]*front[0]+ cadherin[cadherin_index].vector[1]*front[1];//front is a unit vector, [1,0]
...
void Cell::calculate_integrin_force(int integrin_index) { // WHAT IS THE POINT OF THIS FUNCTION???
	return;
}
...
void Cell::calculate_integrin_front_indicator(int integrin_index) {
	double tmp = integrin[integrin_index].vector[0]*front[0]+ integrin[integrin_index].vector[1]*front[1];
...
//gets
void Cell::get_center(double value[], int lim) { // WHAT DOES THIS FUNCTION ACCOMPLISH?
	for(int i = 0; i < lim; i++) {
		value[i] = center[i];
	}
	return;
...
void Cell::get_front(double value[], int lim) { // WHAT DOES THIS FUNCTION ACCOMPLISH?
	for(int i = 0; i < lim; i++) {
		value[i] = front[i];
	}
	return;
...
// get cadherin stuff
void Cell::get_cadherin_location(int cadherin_index, double value[], int lim) { // WHAT DOES THIS FUNCTION ACCOMPLISH?
	for(int i = 0; i < lim; i++) {
...
double Cell::get_cadherin_length(int cadherin_index) { 	
	...
	return cadherin[cadherin_index].length;
}

int Cell::get_ncadherin() const {
	return ncadherin;
}

double Cell::get_cadherin_force(int cadherin_index) const {
	return cadherin[cadherin_index].force;
}
...
double Cell::get_cadherin_mu(int cadherin_index) const {
	return cadherin[cadherin_index].mu;
}

int Cell::get_cadherin_attach(int cadherin_index) const {
	return cadherin[cadherin_index].attach;
}

double Cell::get_cadherin_time(int cadherin_index) const {
	return cadherin[cadherin_index].time;
}

void Cell::get_cadherin_vector(int cadherin_index, double value[], int lim) { // WHAT DOES THIS FUNCTION ACCOMPLISH?
	for(int i = 0; i < lim; i++) {
...
int Cell::get_cadherin_front_indicator(int cadherin_index) {
	...
	return cadherin[cadherin_index].front_indicator;
}

//get integrin stuff
void Cell::get_integrin_location(int integrin_index, double value[], int lim) { // WHAT DOES THIS FUNCTION ACCOMPLISH?
	for(int i = 0; i < lim; i++) {
...
double Cell::get_integrin_length(const int integrin_index) const {
	return integrin[integrin_index].length;
}

int Cell::get_nintegrin() const {
	return nintegrin;
}

double Cell::get_integrin_force(int integrin_index) const {
	return integrin[integrin_index].force;
}

double Cell::get_integrin_kspring(int integrin_index) const {
	return integrin[integrin_index].kspring;
}

double Cell::get_integrin_mu(int integrin_index) const {
	return integrin[integrin_index].mu;
}

int Cell::get_integrin_attach(int integrin_index) const {
	return integrin[integrin_index].attach;
}

double Cell::get_integrin_time(int integrin_index) const {
	return integrin[integrin_index].time;
}

void Cell::get_integrin_vector(int integrin_index, double value[], int lim) { // WHAT DOES THIS FUNCTION ACCOMPLISH?
	for (int i=0; i < lim; i++) {
...
int Cell::get_integrin_front_indicator(int integrin_index) const {
	return integrin[integrin_index].front_indicator;
}
...
void Cell::update_cadherin_attachment_initial(int cadherin_index, double dt, double average_spore, double average_stalk[], double average_different, int restart, double average_unbound_cad_spore, double average_unbound_cad_stalk, int cell_index, Cell *cell) {
	...
	if(rg.Random() < prob_attach_initial) { // decide cadherin binds
					...
					if(icell != cell_index) {//can try to bind  
						...
						for(int iin = 0; iin < ncad; iin++) {
							if(cell[icell].get_cadherin_attach(iin) == -1) {
								...
								int cellindextype = cell[cell_index].get_type();
		...
		int icelltype;
		icelltype=cell[cell_index].get_type(); //0 is spore cell, 1 is stalk 
...
void Cell::update_cadherin_attachment_new(int cadherin_index, double dt,double t, double average_spore,double average_stalk[],double average_different, int restart, double average_unbound_cad_spore, double average_unbound_cad_stalk, int cell_index, Cell *cell) {
	Cadherin& cur_int = cadherin[cadherin_index];
	int iicelltype = cell[cell_index].get_type(); //0 is spore cell, 1 is stalk
	// generate a random number from 0 to 1 from a uniform distribution
	double temprand = rg.Random();  // random number between 0 and 1  
	...
	double new_length = rg.Random()*basic_length_cad +5 +7; // make length random adding onto basic length of 5 microns
	double change_dir = 1;
	double value[2]; // I think this is a vector with x and y coordinates of the front of the cell
	get_front(value, 2);
	double frontdeg;
	...
	int icelltype = cell[cell_index].get_type(); //0 is spore cell, 1 is stalk 
	if(icelltype == 0) { //if cell is green, spore
		cur_int.time = sto.Poisson(average_unbound_cad_spore);// find new attach time average_time  is in seconds 
	}
	else {
...
void Cell::update_integrin_attachment_initial(int integrin_index, double dt, double average_substrate_spore, double average_substrate_stalk[],int restart, double average_unbound_int_spore,double average_unbound_int_stalk, int cell_index, Cell *cell) {
	...
	double newy = cur_int.location[1];
	int cell_indextype;
	cell_indextype=cell[cell_index].get_type();
	//Set a certain portion to stay unattached.  
...
void Cell::print_output(ofstream& fout, int n, double dt) {
	...
	*/
	...
	fout << celltype << "\n"; // Print the cell type
	for(int i = 0; i < ncad; i++) { // Print the cadherin information
		calculate_cadherin_front_indicator(i); // WHY DO WE CALCULATE CADHERIN FRONT INDICATOR BUT NOT INTEGRIN IN THIS FUNCTION????
		double vector[2];
		get_cadherin_location(i, vector,2) ;    
		fout << vector[0] << " " << vector[1] << " " << cadherin[i].attach << " " << cadherin[i].front_indicator << "\n";
	}
	for (int i = 0; i < nint; i++) { // Print the integrin information
		double vector[2];
		get_integrin_location(i, vector,2);    
		fout << vector[0] << " " << vector[1] << " " << integrin[i].attach << " " << integrin[i].front_indicator << "\n";
	}
	return;
}
